...
export class Lights {
    ...
    clusteredLightsStorageBuffer: GPUBuffer;
    ...
    constructor(camera: Camera) {
        ...
        const numClustersX = Math.ceil(
            canvas.width / shaders.constants.clusterSize
        );
        const numClustersY = Math.ceil(
            canvas.height / shaders.constants.clusterSize
        );
        this.numClusters =
            numClustersX * numClustersY * shaders.constants.numClustersZ;

        this.clusteredLightsStorageBuffer = device.createBuffer({
            label: "clustered lights",
            size:
                16 +
                shaders.constants.numLightsPerCluster * this.numClusters * 4, // 16 for numClusters + padding, 4 is size of uint32
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });

        // write num lights
        device.queue.writeBuffer(
            this.clusteredLightsStorageBuffer,
            0,
            new Uint32Array([numClustersX, numClustersY])
        );
        // clustered light ihdices don't need initialization
        ...
        this.clusterLightsComputeBindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                {
                    binding: 1,
                    resource: { buffer: this.clusteredLightsStorageBuffer },
                },
            ],
        });
        ...
        this.clusterLightsComputePipeline = device.createComputePipeline({
            ...
            layout: device.createPipelineLayout({
                ...
                bindGroupLayouts: [
                    ...
                    this.clusterLightsComputeBindGroupLayout,
                ],
            }),
            compute: {
                module: device.createShaderModule({
    ...
    doLightClustering(encoder: GPUCommandEncoder) {
        ...
        const workgroupCount = Math.ceil(
            this.numClusters / shaders.constants.clusterLightsWorkgroupSize
        );
