...
fn main(in: FragmentInput) -> @location(0) vec4f
{
    ...
    let packed = textureLoad(gBufferTex, vec2<i32>(in.fragPos.xy), 0);

    let diffuseColor = unpack4x8unorm(packed.x);
    let normal = decodeOct(unpack2x16snorm(packed.y));
    let depth = bitcast<f32>(packed.z);
